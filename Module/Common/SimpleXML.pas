 ...
 ************************************************************
  ...
  (c) Copyrights 2002, 2003 Michail Vlasov.
    ...
    What's new:
    03-Dec-2009 so V1.1 - Support for Delphi 2007/2009
    ...
    23-Dec-2009 so V1.2 - CloneNode copy data
    24-Dec-2009 so V1.3 - Improved performance for parsing
    27-Dec-2009 so V1.4 - Improved performance for hash and save
    03-Jan-2010 so V1.5 - Minor bugs in russian error messages fixed
    14-AUG-2010 so V1.6 - modify for wlz, Minor bugs in TXmlSaver.SaveToBuffer
...
begin
  case v.VType of
    ...
    begin
      ...
      try
        Result := BinToBase64(p^, VarArrayHighBound(Variant(v), 1) -
                    VarArrayLowBound(Variant(v), 1) + 1, 0);
      finally
...
type
  ...
  private
    ...
    FNames: TXmlNameTable; // FNames - таблица имен. Задается извне
    FAttrCount: Integer;   // Количество атрибутов в массиве FAttrs
    FAttrs: array of TXmlAttrData; // Массив атрибутов
    FChilds: TXmlNodeList; // Список дочерних узлов
    {$IFDEF ADDebug}
    FDebugId: Integer;
    {$ENDIF}
...
begin
  ...
  then begin
    ...
    then begin
      ...
      aNode := aClone.GetObject as TXmlNode;
...
procedure TXmlNodeList.ParseXML(aXML: TXmlSource; aNames: TXmlNameTable;
  ...
  begin
    ...
    Insert(aNode, -1);
    if aNode.FTargetNameID = aNames.FXmlNameID then
    begin
      aXml.ParseAttrs(aNode);
      aXml.ExpectText('?>');
      if aXML.AutoCodepage then
      begin
        EncodingValue := aNode.GetVarAttr(aNames.FEncodingNameId, '');
        if EncodingValue <> '' then
        begin
...
begin
  ...
  begin
    ...
    for i := 0 to FAttrCount - 1 do
    begin
      Result := Result + ' ' + FNames.GetName(a.NameID) + '="'
        + TextToXML(VarToXSTR(TVarData(a.Value))) + '"';
      Inc(a);
    end;
  end;
end;
...
begin
  aTag := FNames.GetName(FNameID);
  if GetOwnerDocument.Get_PreserveWhiteSpace then
  begin
    if Assigned(FChilds) and (FChilds.FCount > 0) then
    begin
  ...
  begin
    ...
    begin
      ...
      FChilds.SaveXML(aXMLSaver);
      if (FChilds.FCount > 1) or (FChilds.FItems[0] is TXmlElement) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          {$IFEND}
          if (Size > 0) and (Size<=SizeOf(FRemainBuff)) then
          begin
            dec(P, Size);
        ...
        begin
          {$IF Defined(XML_WIDE_CHARS) or Defined(Unicode)}
          FBufSize := MultiByteToWideChar(FCodepage, 0, @TempSrc, Size,
            FBuffer, SourceBufferSize);
          {$ELSE}
          FBufSize := MultiByteToWideChar(FCodepage, 0, @TempSrc, Size,
            @TempDest, SourceBufferSize);
          {$IFEND}
        end;
      end;
      {$IF not Defined(XML_WIDE_CHARS) and not Defined(Unicode)}
      FBufSize := WideCharToMultiByte(CP_ACP, 0, @TempDest, FBufSize, FBuffer,
        SourceBufferSize, nil, nil);
      {$IFEND}
    end;
    if FBufSize = 0 then
      FBufSize := -1;
...
begin
  ...
  else if FBufSize = 0 then   //缓冲数据处理完毕，读下一数据
  begin
    FillBuffer;
    Result := Next;
  end
...
begin
  ...
  begin
    ...
    ExpectChar('=');
    SkipBlanks;
    if EOF then raise Exception.Create(SSimpleXmlError20);
...
begin
  if Byte(c) and $80 = 0 then
    Result := 1
  else if Byte(c) and $E0 = $C0 then
    Result := 2
  else if Byte(c) and $F0 = $E0 then
    Result := 3
  else if Byte(c) and $F8 = $F0 then
    Result := 4
  else
    Result := 0;
end;
...
//修改：wlz
//日期：2010年8月14日
//描述：FBufferPtr 为缓冲区的地址，每次将 XmlStr 以 FBufferPtr 保存进缓冲区，
//      之后 FBufferPtr + L。
//      若 XmlStr 的长度 L 大于缓冲区剩余空间 (FRemain) 的三份一，即调用
//      FlushBuffer 写入文件，随即清空缓冲区。
//修正：L > P < 2p 时，会产生剩余数据不写入。
...
{$IFEND}
begin
  //若 L 大于缓冲区剩余空间 (FRemain) 的三份一，即写入一次文件。
  if L > (FRemain div 3) then
    FlushBuffer;
  P := FBuffersize div 3;

  if L > P then
  begin
    //如果 XmlStr 还有剩余 (L > 0)，则递归调用继续处理。
    while L > 0 do
    begin
      if L < P then P := L;
...
//      FlushBuffer;
  ...
  begin
    ...
    {$ELSE}//转换 XmlStr，写入 FBufferPtr，返回值为写入的字节数
...
//写入文件，FRemain 缓冲区剩余空间，缓冲区数据大小：FBuffersize - FRemain
procedure TXmlStmSaver.FlushBuffer;
begin
  if FRemain < FBuffersize then
  begin
