...
uses SynZip, SynWebReqRes, uConfig;
...
begin
 ...
 // FIniFile.Free;
  ...
  FHttpServer.AddUrl(StringTOUTF8(FRoot), StringTOUTF8(FPort), False,
    '+', true);
...
begin
  try
    ...
    try
      if RouteDispatch(LEnv) then
        Result := LEnv.StatusCode
      else if WebBrokerDispatch(LEnv) then
        Result := LEnv.StatusCode 
      else
        Result := 404;
    finally
