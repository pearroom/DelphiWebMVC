...
type
  ...
  private
  public
    function FindFirst(tablename, where: string): ISuperObject; overload; override;
...
begin
  Result := nil;
  if (Trim(tablename) = '') or (Trim(where) = '') then
    Exit;
  if Fields = '' then
    Fields := '*';
...
begin
  Result := nil;
  if (not condb.Connected) or (Trim(select) = '') or (Trim(from) = '') then
    Exit;
  if Trim(order) <> '' then
