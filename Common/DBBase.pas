...
uses
  ...
  FireDAC.Comp.Client, superobject;
...
type
  ...
  public
    ...
    // 查询数据返回到 TFDQuery 变量中
    function Query(sql: string): ISuperObject;
...
begin
  ...
  try
    ...
    TMP_CDS.Close();
    TMP_CDS.sql.Text := 'select ' + Fields + ' from ' + tablename + ' where 1=2';
    TMP_CDS.Open();
    Fields := '';
    TMP_CDS.Append;
    Result := TMP_CDS;
...
function TDBBase.connDB: Boolean;
begin
  if db_start then
  begin
    try
...
begin
  ...
  try
    TMP_CDS.Connection := condb;
    TMP_CDS.Close();
    TMP_CDS.sql.Text := 'select ' + Fields + ' from ' + tablename + ' where ' + key + ' = ' + value;
    TMP_CDS.Open();
    Fields := '';
